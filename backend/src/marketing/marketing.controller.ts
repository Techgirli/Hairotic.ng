...
export class MarketingController {
  ...
  async submitContact(@Body() body: any) {
    if (!body.name || !body.email || !body.message) {
      throw new BadRequestException('Name, email, and message content are required');
    }
  ...
  async createFaq(@Body() body: any) {
    if (!body.question || !body.answer || !body.category) {
      throw new BadRequestException('Question, answer, and category fields are required');
    }
